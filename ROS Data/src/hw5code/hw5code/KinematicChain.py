...
class KinematicChain():
    ...
    def fkin(self, q):
        ...
        for s in self.steps:
            # Note the step s is of type KinematicStep and contains:
        ...
        for s in self.steps:
            # From the above, the step now includes:
              # s.p     Position w.r.t. world
              # s.e     Joint axis w.r.t. world
            
            # Take action based on the joint type: Fill in the
            # appropriate Jacobian column (data w.r.t. world frame).
            if s.type is Joint.REVOLUTE:
                # Revolute is a rotation:
                # p_diff = ep(ptip, s.p)
                # Jv[:,s.dof:s.dof+1] = cross(s.elocal, p_diff)
                # Jw[:,s.dof:s.dof+1] = s.elocal
                Jv[:, s.dof:s.dof+1] = cross(s.e, ptip - s.p)
                Jw[:, s.dof:s.dof+1] = s.e
            elif s.type is Joint.LINEAR:
                # Linear is a translation:
                # Jv[:,s.dof:s.dof+1] = s.elocal
